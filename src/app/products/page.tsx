            ...
            <div className="grid grid- cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {cat.items.map((product) => (
                <div key={product.id} className="bg-zinc-800/50 border border-zinc-700 rounded-lg overflow-hidden hover:border-orange-500/50 transition- all group">
                  {/* Product placeholder */}
                  <div className="relative aspect-square bg-gradient-to-br from-zinc-700 to-zinc-800">
                    <div className="absolute inset-0 flex items-center justify-center"> 
                      <div className="text-center">
                        <div className="font-oswald text-2xl font-bold text-zinc-600 mb-1">
                          {product.id.split('-')[0].toUpperCase()}
                        </div>
                        <div className="font-source-sans text-xs text-zinc-500">{product.category}</div>
                      </div>
                    </div>
                  </div>
                  <div className="p-4">
